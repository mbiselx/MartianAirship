...
% m_env               = 0.04275 * S(balloon.radius);      % [kg] envelope mass
m_env               = 432;      % [kg] envelope mass
m_g                 = 39;                   % [kg] lifting gas mass
m_s                 = 0;                    % [kg] solar panel mass

balloon.mass        = m_env + m_g + m_s;
balloon.inertia     = 2*(m_env/3 + m_g/5)*balloon.radius^2;
...
batteries           = struct();
batteries.capacity  = 12e3 + 3.2e3 + 1.2e3; % [kWh]
batteries.mass      = batteries.capacity/160; % [kg]
...
save("./data/airship.mat", "airship")
disp("airship saved to : './data/airship.mat'")
